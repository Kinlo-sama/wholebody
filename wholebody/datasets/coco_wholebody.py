...
class COCOWholeBodyDataset(BasePoseDataset):
    ...
    def load_annotations(self) -> List[Dict[str, Any]]:
        ...
        # SI USAMOS UN BBOX_FILE (Detecciones de YOLO / Faster R-CNN)
        if self.bbox_file is not None:
            logger.info(f"Cargando bounding boxes pre-detectados desde {self.bbox_file}...")
            with open(self.bbox_file, 'r') as f:
                det_data = json.load(f)
            
            for ann in det_data:
                # Omitir detecciones con un score muy bajo (opcional, MMPose no lo hace siempre, pero es buena práctica)
                if ann.get('score', 1.0) < 0.0:
                    continue
                ...
                bbox_score = float(ann.get('score', 1.0))
        ...
        # SI USAMOS GROUND TRUTH (Entrenamiento o validación Oráculo)
        else:
